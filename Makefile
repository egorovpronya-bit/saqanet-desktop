...
generate_kotlin_protos: 
	...
	# protoc \
	# 	--proto_path=SAQANet-core/ \
	# 	--java_out=./android/app/src/main/java/ \
	# 	--grpc-java_out=./android/app/src/main/java/ \
	# 	$(shell find SAQANet-core/v2 SAQANet-core/extension -name "*.proto")
	rsync -av --delete \
		--include='*/' \
		--include='*.proto' \
		--exclude='*' \
		SAQANet-core/v2 SAQANet-core/extension ./android/app/src/main/protos/
	# # Find .proto files and update package declarations
	# find "./android/app/src/main/java/com/SAQANet/SAQANet/protos" -type f -name "*.java" | while read -r proto_file; do \
	#     if grep -q "^package " "$$proto_file"; then \
	#         $(SED) 's/^package \([\w\.]*\)/package com.SAQANet.SAQANet.protos.\1/g' "$$proto_file"; \
	#     fi \
	# done

generate_go_protoc:
	make -C SAQANet-core -f Makefile protos
	echo "SED: $(SED)"
generate_dart_protoc:
	mkdir -p lib/SAQANetcore/generated
	protoc --dart_out=grpc:lib/SAQANetcore/generated --proto_path=SAQANet-core/  $(shell find SAQANet-core/v2 SAQANet-core/extension -name "*.proto") 	google/protobuf/timestamp.proto ; \
...
linux-appimage-release:
	...
	$(BLUE)Adding StartupWMClass to SAQANet.desktop$(DONE); \
	sed -i '/^\[Desktop Entry\]/a StartupWMClass=app.SAQANet.com' "squashfs-root/SAQANet.desktop"; \
	...
	ARCH=x86_64 appimagetool --no-appstream squashfs-root SAQANet.AppImage > /dev/null; \
	$(BLUE)Cleaning up squashfs$(DONE); \
	rm -rf squashfs-root; \
	$(YELLOW)Creating Portable Package$(DONE); \
	PKG_DIR_NAME="SAQANet-linux-appimage"; \
	$(BLUE)Creating dir: $$PKG_DIR_NAME$(DONE); \
	mkdir -p "$$PKG_DIR_NAME"; \
	$(BLUE)Moving SAQANet.AppImage$(DONE); \
	cp -p "SAQANet.AppImage" "$$PKG_DIR_NAME/SAQANet.AppImage"; \
	$(BLUE)Creating Portable Home directory$(DONE); \
	mkdir -p "$$PKG_DIR_NAME/SAQANet.AppImage.home"; \
...
DOCKER_IMAGE_NAME := SAQANet-linux-builder
DOCKER_FLUTTER_VOL := SAQANet-flutter-sdk-cache
DOCKER_PUB_VOL := SAQANet-pub-cache
...
ios-libs: #not tested
	mkdir -p $(IOS_OUT)
	rm -rf $(IOS_OUT)/SAQANetCore.xcframework
...
build-headers:
	make -C SAQANet-core -f Makefile headers && mv $(BINDIR)/$(CORE_NAME)-headers.h $(BINDIR)/SAQANet-core.h

build-android-libs:
	make -C SAQANet-core -f Makefile android 
	mv $(BINDIR)/$(LIB_NAME).aar $(ANDROID_OUT)/

build-windows-libs:
	make -C SAQANet-core -f Makefile windows-amd64

build-linux-libs:
	make -C SAQANet-core -f Makefile linux-amd64 

build-macos-libs:
	make -C SAQANet-core -f Makefile macos

build-ios-libs: 
	rm -rf $(IOS_OUT)/SAQANetCore.xcframework 
	make -C SAQANet-core -f Makefile ios  
	mv $(BINDIR)/SAQANetCore.xcframework $(IOS_OUT)/SAQANetCore.xcframework
